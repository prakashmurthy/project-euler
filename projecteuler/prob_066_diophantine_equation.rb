...
(2..MAX_NUMBER).each do |number|
  ...
  puts "number is #{number}. Solution is #{minimal_solution}. Time spent is #{(Time.now - start_time) / 60 }  minutes"
end

puts "\n\nThe max value of x when D <= #{MAX_NUMBER} is #{max_x} for D = #{d_with_max_x}"
# puts 13.minimal_solution_for_x
